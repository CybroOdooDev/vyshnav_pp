...
class ResConfigSettings(models.TransientModel):
    ...
    Methods:
        ...
        _find_attachment_type(attachment):
    ...
    @api.model
    def get_values(self):
        """ Adding the new config parameter values to the dictionary
        Returns:
            dict: dictionary of config parameters.
        """
        res = super().get_values()
        ir_config_parameter = self.env['ir.config_parameter'].sudo()
        res['url'] = ir_config_parameter.get_param('url')
        res['user_id_jira'] = ir_config_parameter.get_param('user_id_jira')
        res['api_token'] = ir_config_parameter.get_param('api_token')
        res['export_project_count'] = ir_config_parameter.get_param(
            'export_project_count')
        res['export_task_count'] = ir_config_parameter.get_param(
            'export_task_count')
        res['import_project_count'] = ir_config_parameter.get_param(
            'import_project_count')
        res['import_task_count'] = ir_config_parameter.get_param(
            'import_task_count')
        res['connection'] = ir_config_parameter.get_param(
            'odoo_jira_connector.connection')
        return res

    @api.model
    def set_values(self):
        """ Set the values in the config parameters """
        ir_config_parameter = self.env['ir.config_parameter'].sudo()
        ir_config_parameter.set_param('url', self.url)
        ir_config_parameter.set_param('user_id_jira', self.user_id_jira)
        ir_config_parameter.set_param('api_token', self.api_token)
        ir_config_parameter.set_param(
            'export_project_count', self.export_project_count)
        ir_config_parameter.set_param(
            'export_task_count', self.export_task_count)
        ir_config_parameter.set_param(
            'import_project_count', self.import_project_count)
        ir_config_parameter.set_param(
            'import_task_count', self.import_task_count)
        ir_config_parameter.set_param(
            'odoo_jira_connector.connection', self.connection)
                    ...
                    data = json.dumps({
                        'body': {
                            ...
                            'content': [{
                                'type': 'paragraph',
                                'content': [{
                                    'text': str(html2plaintext(message.body)),
                                    'type': 'text'}]
                            }]
                        }
                    })
    ...
    def _export_attachments(self, attachments, attachment_url):
        ...
        for attachment in attachments:
            attachment_type = self._find_attachment_type(attachment)
    ...
    def _find_attachment_type(self, attachment):
        """ To find the corresponding attachment type in the attachment model
        Args:
